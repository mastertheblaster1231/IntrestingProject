...
export function SelectedLocationOverlay() {
  ...
  useEffect(() => {
    if (activeInstrument && activeInstrument.lat && activeInstrument.lon) {
      setLocation(prev => ({
        ...prev,
        floatId: activeInstrument.id || activeInstrument.floatId || prev?.floatId || "",
        ...
        depth: activeInstrument.depthMeters || activeInstrument.depth || prev?.depth || 0,
  ...
  const handlePointerDown = (e) => {
    if (e.target.closest('.overlay-controls') || e.target.closest('.search-input')) return;
  ...
  const handleSearch = (e) => {
    ...
    if (typeof window !== "undefined" && window.argoPoints) {
      const match = window.argoPoints.find(p => 
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: isMinimized ? 0 : "10px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <h4 style={{ margin: 0, color: "#fff", fontSize: "0.85rem", fontWeight: 600 }}>
            {location ? (location.sea || "Selected Location") : "No Selection"}
          </h4>
          {location?.status && (
            <span style={{
              fontSize: "0.55rem", padding: "2px 5px", borderRadius: "4px",
              background: location.status === "active" ? "rgba(74,222,128,0.15)" : "rgba(250,204,21,0.15)",
        ...
        <div className="overlay-controls" style={{ display: "flex", gap: "4px" }}>
          <button
      ...
      {!isMinimized && (
        <>
          {/* Location Details */}
          {location ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "5px", cursor: "default", marginBottom: "10px" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#94a3b8" }}>Float ID:</span>
                <span style={{ fontWeight: 600, color: "#38bdf8", fontFamily: "JetBrains Mono, monospace" }}>{location.floatId || "--"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#94a3b8" }}>Region:</span>
                <span style={{ fontWeight: 500 }}>{location.region || location.sea || "--"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#94a3b8" }}>Latitude:</span>
                <span style={{ fontWeight: 500, fontFamily: "JetBrains Mono, monospace" }}>{formatCoord(location.lat, true)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#94a3b8" }}>Longitude:</span>
                <span style={{ fontWeight: 500, fontFamily: "JetBrains Mono, monospace" }}>{formatCoord(location.lon, false)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "#94a3b8" }}>Max Depth:</span>
                <span style={{ fontWeight: 500 }}>{location.depth || "--"} m</span>
              </div>
            </div>
          ) : (
            <div style={{ padding: "12px 0", textAlign: "center", color: "#64748b", fontStyle: "italic", cursor: "default" }}>
