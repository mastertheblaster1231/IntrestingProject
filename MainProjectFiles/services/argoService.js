...
// ─── ERDDAP ROW INDEX MAP ────────────────────────────────────────────────────
// ERDDAP returns rows as ordered arrays. These indices match the variable
// order in the query string: platform_number,time,latitude,longitude,pres,temp,psal
const COL = {
  PLATFORM: 0,
  TIME:     1,
  LAT:      2,
...
async function _fetchFromERDDAP(id) {
  ...
  return _parseErddapRows(id, rows);
}
...
function _parseErddapRows(id, rows) {
  ...
  // Build the vertical profile array from all pressure-level rows
  const profile = rows
    .map((row) => {
 ...
 * --------------------
 * Loads the bundled real_argo_cache.json and returns the entry matching `id`.
 * If the specific float is not in the cache, generates a minimal synthetic profile
...
async function _fetchFromLocalCache(id) {
  try {
    ...
    if (!cacheResponse.ok) throw new Error(`Cache fetch HTTP ${cacheResponse.status}`);
    ...
    console.warn(`[argoService] Float #${id} not in offline cache; using synthetic fallback`);
 ...
 * _syntheticFallback
 * ------------------
 * Generates a physically plausible Indian Ocean profile from analytic equations.
 * This is the absolute last resort so the 3D visualization NEVER shows blank panels.
 *
 * Temperature model: exponential thermocline decay (classic Bryden 1973 approximation)
 * Salinity model:    subsurface maximum at ~150 m (typical Arabian Sea/Bay of Bengal)
 *
 * @param {string} id  WMO platform number
 * @returns {ArgoProfile}
 */
function _syntheticFallback(id) {
  ...
  const profile = DEPTHS.map((d) => {
    // Thermocline: exponential decay from surface warm layer (~28 degrees C) to deep water (~2 degrees C)
    let temperature;
    if (d <= 50) {
      temperature = surfaceT - (d / 50) * 0.4;
    } else if (d <= 1000) {
      temperature = 3.5 + (surfaceT - 3.5) * Math.exp(-d / 320);
    } else {
      temperature = 1.8 + (3.5 - 1.8) * Math.exp(-(d - 1000) / 900);
    }

    // Halocline: subsurface salinity maximum around 150 m (common in Indian Ocean)
    let salinity;
    if (d <= 150) {
